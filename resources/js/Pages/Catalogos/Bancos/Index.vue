...
const askDelete = (item) => {
    ...
    confirmModal.value.show({
        ...
        message: `¿Estás seguro de eliminar el banco "${item.nombre}"? Esta acción no se puede deshacer.`,
...
const deleteItem = () => {
    if (itemToDelete.value) {
        router.delete(route('bancos.destroy', itemToDelete.value.id), {
            preserveScroll: true,
        });
...
<template>
    ...
    <AppLayout>
        <template #header>Bancos</template>

        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-zinc-900">Gestión de Bancos</h1>
                <p class="mt-1 text-sm text-zinc-600">
                    Administra el catálogo de instituciones bancarias
                </p>
            </div>
            <Link
            ...
            <div class="grid gap-4 md:grid-cols-3">
                ...
                <div class="flex items-end">
                    <div class="rounded-lg bg-zinc-100 px-4 py-2">
                        <p class="text-xs text-zinc-600">Total de bancos</p>
                        <p class="text-2xl font-bold text-zinc-900">{{ bancos.total }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-zinc-200 bg-white shadow">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-zinc-200 bg-zinc-50">
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500">SWIFT</th>
                            <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500">País</th>
                        ...
                        >
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <Link
                                        :href="route('bancos.edit', item.id)"
                        ...
                        <tr v-if="bancos.data.length === 0">
                            <td colspan="6" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-zinc-500">No se encontraron bancos</p>
                                    <Link
                                        :href="route('bancos.create')"
                                        class="mt-2 text-sm text-[#7c40ff] hover:underline"
                                    >
                                        Registrar el primer banco
                                    </Link>
                                </div>
                            </td>
                        </tr>
            ...
            <div v-if="bancos.data.length > 0" class="border-t border-zinc-200 px-6 py-4">
                <div class="flex items-center justify-between">
                    <p class="text-sm text-zinc-600">
                        Mostrando {{ bancos.from }} a {{ bancos.to }} de {{ bancos.total }} bancos
                    </p>
                    <div class="flex gap-2">
                        <template v-for="link in bancos.links" :key="link.label">
                            <Link
                                v-if="link.url"
                                :href="link.url"
                                :class="[
