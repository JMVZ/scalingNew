...
const operadorToDelete = ref(null);

watch([search, activo], () => {
    router.get(route('operadores.index'), {
        search: search.value,
...
const askDeleteOperador = (operador) => {
    operadorToDelete.value = operador;
    confirmModal.value.show({
        title: '¿Eliminar Operador?',
        message: `¿Estás seguro de eliminar al operador "${operador.nombres} ${operador.apellido_paterno}"? Esta acción no se puede deshacer.`,
        onConfirm: () => deleteOperador()
    });
};

const deleteOperador = () => {
    if (operadorToDelete.value) {
        router.delete(route('operadores.destroy', operadorToDelete.value.id), {
            preserveScroll: true,
        });
        operadorToDelete.value = null;
    }
};
</script>
...
<template>
    ...
    <AppLayout>
        <template #header>Operadores</template>

        <div class="mb-6 flex items-center justify-between">
            <div>
                <h1 class="text-3xl font-bold text-zinc-900">Gestión de Operadores</h1>
                <p class="mt-1 text-sm text-zinc-600">
                    Administra el catálogo de operadores de tu empresa
                </p>
            </div>
            <Link
            ...
            <div class="grid gap-4 md:grid-cols-3">
                <div>
                    ...
                    <input
                        v-model="search"
                        type="text"
                        placeholder="Número, nombre, licencia..."
                ...
                <div class="flex items-end">
                    <div class="rounded-lg bg-zinc-100 px-4 py-2">
                        <p class="text-xs text-zinc-600">Total de operadores</p>
                        <p class="text-2xl font-bold text-zinc-900">{{ operadores.total }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="rounded-2xl border border-zinc-200 bg-white shadow">
                ...
                <table class="w-full">
                    <thead>
                        <tr class="border-b border-zinc-200 bg-zinc-50">
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500">Nombre Completo</th>
                            ...
                            <th class="px-6 py-3 text-left text-xs font-medium text-zinc-500">Licencia</th>
                    ...
                    <tbody>
                        <tr
                            v-for="operador in operadores.data"
                            :key="operador.id"
                            class="border-b border-zinc-100 hover:bg-zinc-50"
                        >
                            <td class="px-6 py-4">
                                <p class="font-medium text-zinc-900">{{ operador.numero_operador }}</p>
                            </td>
                            <td class="px-6 py-4">
                                <p class="font-medium text-zinc-900">
                                    {{ operador.nombres }} {{ operador.apellido_paterno }} {{ operador.apellido_materno || '' }}
                                </p>
                            </td>
                            ...
                            <td class="px-6 py-4">
                                <div class="flex items-center justify-end gap-2">
                                    <Link
                                        :href="route('operadores.edit', operador.id)"
                                    ...
                                    <button
                                        @click="askDeleteOperador(operador)"
                        ...
                        <tr v-if="operadores.data.length === 0">
                            <td colspan="7" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center gap-2">
                                    ...
                                    <p class="text-zinc-500">No se encontraron operadores</p>
                                    <Link
                                        :href="route('operadores.create')"
            ...
            <div v-if="operadores.data.length > 0" class="border-t border-zinc-200 px-6 py-4">
                <div class="flex items-center justify-between">
                    <p class="text-sm text-zinc-600">
                        Mostrando {{ operadores.from }} a {{ operadores.to }} de {{ operadores.total }} operadores
                    </p>
                    <div class="flex gap-2">
                        <template v-for="link in operadores.links" :key="link.label">
                            <Link
                                v-if="link.url"
                                :href="link.url"
                                :class="[
